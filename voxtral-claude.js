...
// Voxtral is Mistral’s speech model family; transcription is exposed via the /v1/audio/transcriptions endpoint using the voxtral-mini-latest (aka Voxtral Mini Transcribe) model. 

// https://docs.mistral.ai/capabilities/audio/
// https://console.mistral.ai/usage

//  node voxtral-claude.js --chunk-sec=60 --silence-aware test

/**
 * Voxtral batch SRT — Enhanced with improved timing accuracy and transcription quality
 * 
 * Usage:
...
/**
 * CORRECTED VERSION - Fixes timestamp drift logic error
 * 
 * Key fix: Proper timestamp offset calculation without double-counting
 */
...
/* ---------------- Main execution ---------------- */
async function main() {
  console.log(`🚀 Voxtral Enhanced - CORRECTED TIMESTAMP VERSION`);
  ...
  try {
    ...
    if (stat.isFile()) {
      ...
      console.log(`📂 Scanning directory for video files: ${inputPath}`);
    ...
    videoFiles.forEach((file, i) => {
      ...
      console.log(`  ${i + 1}. ${path.basename(file)} → ${path.basename(enhancedSrtPath)}`);
    });
    ...
    for (const videoFile of videoFiles) {
      try {
        ...
        console.error(`❌ Failed: ${path.basename(videoFile)} - ${err.message}`);
        failed++;
      }
    }
